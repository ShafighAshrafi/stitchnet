...
def draw_net(stitchNet, name="_results_with_finetune/stitchnet/net"):
    ...
    for i, nId in enumerate(nIds):
        hexColor[i] = colors[
            i % len(colors)
        ]  # rgb_to_hex(*[int(c*255) for c in colors[i%len(colors)]])
...
def draw_stitchNet_fromTuples(
    fragmentTuples, numNets=10, name="_results_with_finetune/stitchnet/net"
):
    hexColor = {}
