...
from src.service.finetune.train_model_last_layer import train_model_last_layer
from src.utilities.set_requires_grad_parameters import set_parameter_requires_grad


def finetune_after_stitching(model, num_classes=3, num_epochs=3, batch_size=64, val_batch_size=128, feature_extracting=False):
    ...
    setattr(model, module_name, nn.Linear(module.in_features,
                    num_classes, bias=module.bias is not None))
    # model = torch.nn.Sequential(model, torch.nn.Linear(
    #     module.in_features, num_classes, bias=module.bias is not None))
    # torch.onnx.export(model, torch.ones(1, 3, 224, 224), f'test.onnx')
    ...
    criterion = nn.CrossEntropyLoss(weight=torch.tensor(
        [total_train_data / count for count in dataset_info.values()])).cuda()
    # Train and evaluate
