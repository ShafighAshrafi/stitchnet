...
def check_fragment_exists(curr, nextf):
    fragment_id = curr.get_id()
    next_fragment_id = nextf.get_id()
...
def find_next_fragment(curr, scoreMapper, data, threshold=0.5, maxDepth=10, sample=False, K=None):
    ...
    scores = [(s,nextf) for s,nextf in scores if not check_fragment_exists(curr,nextf)]
